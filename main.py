...
wandb.init(project='SF-Net')

def main():
    feature_dim = int(args.feature_size / 2)
    #wandb.init(project='SF-Net') #, name = args.name, group = args.group)
    ...
    wandb.log({'Model Name': args.model_name})
    ...
    wandb.log({'Alpha': args.alpha,'Beta': args.beta, 'gamma': args.gamma})
    ...
    for itr in range(args.max_iter + 1):
        ...
        if args.expand and itr == args.expand_step:
            ...
            #wandb.config.update({'Parameters':model.parameters()})
            wandb.run.finish()
        if args.eval_only:
            print('Done Eval!')
            break
